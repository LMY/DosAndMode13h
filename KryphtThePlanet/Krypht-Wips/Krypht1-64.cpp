...
int main()
{
char Bin[64], Kry[64], Word[8], Krytmp[64];
int handle, handle1, readed;
char *Key = new char[64];
char *Key2 = new char [64];
char *Key3 = new char [64];
...
if ((handle1 = open("OutPut.txt", O_CREAT | O_BINARY | O_EXCL)) == -1)
	...
	for (int i=0;i<(readed*8);i++) Krytmp[i] = Bin[Key[i]-1];

	Kry[Key[0] -1]  = Krytmp[Key[63]-1];
	for (int i=0;i<63;i++)
		Kry[Key[i+1]-1] = Krytmp[Key[i]-1];
