...
for (int volte = 0 ; volte < Passi ; volte++)
{
	...
	//copia in Key3 la Key che è stata usata (e che nel loop precedente rappresenta quella di due volte
	//prima di kryphtare
	for (int i=0 ; i<64 ; i++)
	{
		Key3[i] = Key[i];
	}
	//copia Key2 in quella che verrà usata per Kryphtare
	for (int i=0 ; i<64 ; i++)
	{
		Key[i] = Key2[i];
	}
