...
void GetPcxFarImg(unsigned char File[],unsigned char far *Image);
...
void PutFarFrame(unsigned char far *Pointer,unsigned int x,unsigned int y,unsigned int Xsize, unsigned int Ysize);
void PutFarFrame(unsigned char far *Pointer,unsigned int x,unsigned int y,unsigned int Xsize, unsigned int Ysize,unsigned int where);
void PcxTile(unsigned char File[],unsigned int where);
void DrawRawImg(unsigned char File[]);
void GetFarRawImg(unsigned char File[],unsigned char far * Pointer);

unsigned char DrawPcx(unsigned int Xoff, unsigned int Yoff, unsigned char File[])
{
	int handle;
	if ((handle = open(File, O_RDONLY | O_BINARY )) == -1) return 0;
	unsigned char *F = new unsigned char[60000];
	read(handle,F,128);
	...
	if ((handle = open(File, O_RDONLY | O_BINARY )) != -1)
	{
		...
		x=0;
			...
			if (F[P]>=192)
				...
				Count=F[P]-192;
				P++;
				for (k=0;k<Count;k++) Image[x+k+Xsize*y]=F[P]; //PutPixel(x+k+Xoff,y+Yoff,F[P]);
				x+=Count;
				}
			else { Image[x+Xsize*y]=F[P]; x++; }
			}
...
void PutFrame(unsigned char Pointer[],unsigned int x,unsigned int y,unsigned int Xsize, unsigned int Ysize)
{
	for (unsigned char a=0;a<Ysize;a++)
...
void PcxTile(unsigned char File[],unsigned int where)
{
	unsigned char far *Image = (unsigned char far *) farcalloc(64000,1);
	int handle;
	...
	for (y=0;y<200;y+=Ysize)
		for (x=0;x<320;x+=Xsize)
			PutFarFrame(Image,x,y,Xsize,Ysize,where);
	farfree(Image);
}
void DrawRawImg(unsigned char File[])
{
